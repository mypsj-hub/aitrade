...
# =============================================================================
# 3. Supabase 연결 및 데이터 로딩 (읽기 전용)
# =============================================================================
@st.cache_resource
def get_supabase_client() -> Client:
    """Supabase 클라이언트 초기화 (읽기 전용 - ANON KEY 사용)"""
    try:
        # Streamlit secrets에서 먼저 읽기 (Vercel 배포용), 없으면 환경 변수
...
def load_portfolio_data() -> Dict:
    """Supabase에서 포트폴리오 데이터 로딩 (읽기 전용)"""
    try:
        supabase = get_supabase_client()

        # 보유 현황 조회
        holdings_response = supabase.table('holding_status').select('*').order('평가금액', desc=True).execute()
        holdings_df = pd.DataFrame(holdings_response.data) if holdings_response.data else pd.DataFrame()

        # 포트폴리오 요약 (최신 1건)
        summary_response = supabase.table('portfolio_summary').select('*').order('날짜', desc=True).limit(1).execute()
        summary_df = pd.DataFrame(summary_response.data) if summary_response.data else pd.DataFrame()

        # 포트폴리오 요약 (전체 - 차트용)
        summary_full_response = supabase.table('portfolio_summary').select('*').order('날짜', desc=False).execute()
        summary_full_df = pd.DataFrame(summary_full_response.data) if summary_full_response.data else pd.DataFrame()

        # CIO 최신 전략 (system_status에서)
        cio_response = supabase.table('system_status').select('status_value').eq('status_key', 'cio_latest_rationale').execute()
        cio_rationale = cio_response.data[0]['status_value'] if cio_response.data else "CIO 브리핑이 아직 생성되지 않았습니다."

        # 거래 기록 (최근 1000건)
        history_response = supabase.table('trade_history').select('*').order('거래일시', desc=True).limit(1000).execute()
        history_df = pd.DataFrame(history_response.data) if history_response.data else pd.DataFrame()

        # CIO 보고서
        reports_response = supabase.table('cio_reports').select('*').order('report_date', desc=True).execute()
        reports_df = pd.DataFrame(reports_response.data) if reports_response.data else pd.DataFrame()
...
def display_key_metrics(summary_data: pd.DataFrame, holdings_data: pd.DataFrame) -> None:
    ...
    cols = st.columns(5)
    cols[0].metric("💰 총 자산", f"₩ {total_value:,.0f}", help="초기 투자 원금")
