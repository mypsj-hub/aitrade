...
│
├── components/                # React 컴포넌트 (총 27개)
│   ├── Navigation.tsx        # 전역 네비게이션
│   ├── PortfolioSummaryCard.tsx
│   ├── HoldingsTable.tsx
│   │
│   ├── [신규 v3.1 컴포넌트] ⭐
│   ├── StrategyComparisonCard.tsx # CIO + 매매전략 비교 카드
│   │
│   ├── [신규 v3.0 컴포넌트]
...
│   ├── api/                  # API 함수
│   │   └── tradeTypes.ts     # 거래 유형 동적 조회 (신규 v3.1) ⭐
│   ├── store/                # Zustand 상태 관리
│   │   └── filterStore.ts
│   ├── hooks/                # 커스텀 훅
│   │   ├── useDashboardData.ts
...
│   └── utils/                # 유틸리티 함수
│       ├── formatters.ts     # 날짜/숫자 포맷 (v3.1 업데이트)
│       └── detectNewCoins.ts # 신규 코인 로직 (신규 v3.0)
│
├── docs/                      # 문서 (Level 3)
│   ├── README.md             # 문서 인덱스
│   └── dev_guide/            # 개발 가이드
...
- **StrategyComparisonCard** ⭐ - CIO + 매매전략 비교 (v3.1 신규)
...
**주요 컴포넌트**:
- **AnalysisFilters** ⭐ - 동적 2단계 필터 (v3.1 업데이트)
- **AnalysisSummary** ⭐ - 5개 메트릭 (총 거래, 청산 거래, 손익, 승률, 손익비) (v3.1 업데이트)
- **PnlByAssetChart** - 코인별 손익 막대 차트
- **PerformanceTrendChart** - 수익률 추세 차트
- **CoinStatsTable** ⭐ - 코인별 상세 통계 (v3.1 업데이트)
- **AIPatternAnalysis** ⭐ - AI 매매 패턴 분석 (3단계 차트) (v3.1 업데이트)
...
- 동적 2단계 거래 유형 필터:
  - 메타 필터: 매수(전체), 매도(전체)
  - 상세 필터: 신규매수, 익절, 손절, 시스템손절 등 (DB 동적 조회)
- 하이브리드 필터링 (LIKE + 정확한 매칭)
- 코인명 실시간 검색
- 행 확장 시 AI 사고과정 + 주요지표 표시
- Nullable 통계 처리 (청산 거래 없을 때 안내 메시지)
...
- **CIODecisionsTable** - CIO 판단 이력 (아코디언 스타일)
  ...
  - 판단 근거 및 타임라인
  - 확장 시 상세 판단 내용 표시
- **CIODecisionTimeline** - 시간순 판단 타임라인
...
**신규 컴포넌트**:
- `StrategyComparisonCard` - CIO + 매매전략 비교 카드
  - 역할 구분: 파란색(CIO) vs 초록색(매매전략)
  - 독립적 펼치기/접기
  - 실제 체결 결과 표시 (execution_summary)

**신규 API 함수**:
- `lib/api/tradeTypes.ts`
  - `getDistinctTradeTypes()` - DB 동적 거래 유형 조회
  - `getColorForTradeType()` - 유형별 색상 클래스 반환

**업데이트된 컴포넌트**:
- `AnalysisFilters` - 동적 2단계 필터 (메타 + 상세)
- `AnalysisSummary` - closedTradesCount prop 추가, nullable 처리
- `AIPatternAnalysis` - 3단계 차트 분리 (분포/손익/시간대)
- `CoinStatsTable` - 청산 거래 없을 때 안내 메시지
- `RecentTradesTable` - 텍스트 전체 표시

**데이터베이스** (backend 영역):
- `system_status.process2_latest_advice` 저장
- `cio_reports.process2_latest_advice` 저장
- `cio_reports.execution_summary` 저장

**기술 개선**:
- TypeScript unknown 경유 타입 단언 (Vercel 빌드 호환)
- KST 시간대 표시 수정
- ESLint 경고 해결 (no-unused-vars)
...
**📝 버전**: v3.1.0 (전략 비교 시스템 및 분석 필터 고도화)
