 ...
 * AI 패턴 분석 컴포넌트
 *
 * 목적: AI의 거래 패턴과 성과를 다각도로 분석하여 시각화하기 위함
 * 역할: 거래유형별 성과와 시간대별 거래 패턴을 차트와 테이블로 표시
 *
 * 주요 기능:
 * - 거래유형별 성과 차트 (거래수, 승률을 이중 Y축으로 표시)
 * - 거래유형별 상세 통계 테이블 (거래수, 승률, 평균/총 손익)
 * - 시간대별 거래 분포 차트
 * - 승률에 따른 막대 색상 자동 변경 (높음: 초록, 중간: 파랑, 낮음: 빨강)
 * - 청산 거래만 필터링하여 정확한 통계 계산
 *
 * Props:
 * - trades: Trade[] - 거래 내역 배열
 *
 * 데이터 소스: trade_history 테이블
 * 기술 스택: Recharts, React useMemo
 */
'use client';
